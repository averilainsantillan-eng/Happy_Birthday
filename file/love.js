(function(window){
    ...
    Seed.prototype = {
        ...
        scale: function(scale) {
            ...
            if (this.textOpacity > 0) {
                this.drawText();
                this.textOpacity = Math.max(0, this.textOpacity - 0.055);
                if (this.textOpacity <= 0) {
                    var ctx = this.tree.ctx;
                    var point = this.cirle.point;
                    ctx.clearRect(point.x - 80, point.y - 80, 420, 160);
                    this.drawCirle();
                    this.drawHeart();
                }
            }
        ...
        drawText: function() {
            if (this.textOpacity <= 0) return;
            var ctx = this.tree.ctx, heart = this.heart;
            var point = heart.point, color = heart.color, 
                scale = heart.scale;
            ctx.save();
            ctx.globalAlpha = Math.max(0, Math.min(1, this.textOpacity));
            ...
            ctx.moveTo(0, 0);
            ctx.scale(0.75, 0.75);
            ctx.font = "12px 'Lucida Handwriting', 'Comic Sans MS', cursive, sans-serif"; 
            ctx.fillText("Click Me:) ", 30, -5);
            ctx.fillText("Birthday Queen !", 28, 11);
            ctx.restore();
        },
        clear: function() {
            var ctx = this.tree.ctx, cirle = this.cirle;
            ...
            if (this.textOpacity > 0) {
                ...
                var scale = cirle.scale, radius = 26;
                var w = radius * scale;
        ...
        hover: function(x, y) {
            ...
            if (ix >= 0 && ix < this.tree.width && iy >= 0 && iy < this.tree.height) {
                var pixel = ctx.getImageData(ix, iy, 1, 1);
                if (pixel.data[3] > 30) return true;
            }
            if (this.heart && this.heart.point) {
                var hp = this.heart.point;
                var dx = x - hp.x;
                var dy = y - hp.y;
    ...
    Bloom = function(tree, point, figure, color, alpha, angle, scale, place, speed) {
        this.tree = tree;
        this.point = point;
        this.color = color || 'hsl(' + random(192, 222) + ', ' + random(75, 95) + '%, ' + random(68, 88) + '%)';
